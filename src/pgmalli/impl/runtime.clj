...
(defn registry
  ...
  (let [base (merge (m/default-schemas) (mu/schemas) (time/schemas)
    ...
    (let [generated (into {} (map (fn [[k s]] [k (walk/postwalk generate-from-part (if (row-schema? s) (with-gen-hints s) s))])) generated)]
...
(defn- data-columns
  ...
  [registry name]
  (let [s (get (schemas-of registry) name)]
...
(defn column
  ...
  (let [k (render/ident-key (clojure.core/name col))]
    (some (fn [[ek _ s]] (when (= k ek) (without-gen s))) (column-entries (get (schemas-of registry) name)))))
...
(defn- portable-node [registry f]
  (let [[t p] (if (vector? f) [(first f) (when (map? (second f)) (second f))] [f nil])]
    (cond
      ...
      (portable-node registry (merge-props (get (schemas-of registry) (last f)) p))
      (and (vector? f) (= :and t))
...
(defn portable
  ...
  [registry name]
  (portable-data registry (get (schemas-of registry) name)))
...
(defn- tables
  ...
  ([registry except]
   (let [ts (for [[k s] (sort-by (comp str key) (schemas-of registry))
...
(defn- fill-branches
  ...
  (let [generate (requiring-resolve 'clojure.test.check.generators/generate)
        schema (get (schemas-of registry) name)
...
(defn- insert-value
  ...
  (let [s (non-null schema)
        ...
        base (if (and (vector? s) (= :ref (first s))) (get (schemas-of registry) (last s)) s)
...
(defn inserts
  ...
  (let [ts (filter #(seq (get dataset (:table %))) (tables registry))]
    ...
    (for [{:keys [name table refs]} (parents-first ts)
          :let [entries (column-entries (get (schemas-of registry) name))
